...
void event_image_detect(struct context *cnt, int type ATTRIBUTE_UNUSED,
        unsigned char *newimg, char *tampered,
        void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
    ...
    if (cnt->new_img & NEWIMG_ON) {
		...
		if(tampered)
        	snprintf(fullfilename, PATH_MAX, "%s/%s%s.%s", cnt->conf.filepath, tampered,filename, imageext(cnt));
		else 
...
void event_ffmpeg_newfile(struct context *cnt, int type ATTRIBUTE_UNUSED,
            unsigned char *img, char *tampered,
            void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
    ...
    if(tampered )
		snprintf(cnt->newfilename, PATH_MAX - 4, "%s/%s%s",cnt->conf.filepath,tampered, stamp);
	else
