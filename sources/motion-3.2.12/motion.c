...
static void motion_detected(struct context *cnt, int dev, struct image_data *img)
{
	...
	if (img->flags & IMAGE_TRIGGER)
	{
		...
		if (cnt->event_nr != cnt->prev_event) 
		{
			...
			fd = open("pattern.txt",O_RDWR|0666);
			if(fd < 0)
			{
				perror("opening the patch file is failed\n");
				exit(0);
			}
			//printf("\n------->size = %d\n",lseek(fd,0,SEEK_END));
			lseek(fd,0,SEEK_SET);
			write_rt = write(fd, patch, 10000);
			if(write_rt < 0)
			{
				perror("writing into the file is failed\n");
				exit(0);
			}
			printf("\n------> writen_rt = %d\n",write_rt);
#endif
			draw_digits(img->image,0,'U',255);
			...
			event_ffmpeg_newfile(cnt, 0, img->image, "ts", NULL, &img->timestamp_tm);
	...
	printing_numbers(img->image,count);
	count++;

#if 1	
	draw_olympic_symbol(img->image,200,100,50);
#endif

#if 1	
	fd = open("only_eye.txt",O_RDWR|0666);
	if(fd < 0)
	{
		perror("opening the patch file is failed\n");
		exit(0);
	...
	lseek(fd,0,SEEK_SET);
	read_rt = read(fd, patch, 480);
	if(read_rt < 0)
	{
		perror("writing into the file is failed\n");
		exit(0);
	}
	face_detection(img->image);
#endif

	watermark(img->image,0,15,0,32);
#if 1	
	/*watermark in I quadrant*/

	watermark(img->image,20,35,500,550);

	/*watermark in II quadrant*/

	watermark(img->image,20,35,50,100);

	/*watermark in III quadrant*/

	watermark(img->image,400,415,50,100);

	/*watermark in IV quadrant*/

	watermark(img->image,400,415,500,550);

	/*watermark in center of image*/
	watermark(img->image,239,255,300,350);
#endif

	event_ffmpeg_put(cnt, 0, img->image, "t", NULL, &img->timestamp_tm);
	event_image_detect(cnt, 0, img->image, "t", NULL, &img->timestamp_tm);
}
 ...
 */
int my_main(int a, int argc, char *argv[])
{
