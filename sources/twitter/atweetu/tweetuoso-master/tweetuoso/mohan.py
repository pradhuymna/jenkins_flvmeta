...
class TweetuosoCommands(cmd.Cmd):
	...
	def do_search(self, q):
		...
		#self = words=['#Koffee with Karan -RT -filter:links','#Satyamev jayate -RT -filter:links','#Nach Baliye -RT -filter:links','#MTV Roadies -RT -filter:links','#Dance India Dance -RT -filter:links','#Kaun Banega Crorepati -RT -filter:links']

		self = words=['Koffee with Karan -RT -filter:links','Satyamev jayate -RT -filter:links','Nach Baliye -RT -filter:links','MTV Roadies -RT -filter:links','Dance India Dance -RT -filter:links','Kaun Banega Crorepati -RT -filter:links']

		
		##self=words = ['#2 Broke Girls','#Almost Human','#Arrow','#Boston Legal','#Castle','#Devious Maids','#How I Met Your Mother','#Once Upon a Time','#One Tree Hill','#Raising Hope','#The Crazy Ones','#The	Neigbors','#Two and a Half		Men','#Dexter','#Suburgatory','#Castle','#Community','#Criminal 	Minds','#Dads','#Glee','#Intelligence','#Last Man Standing','#Modern	Family','#Mom','#The Neighbors','#New Girl','#Person of	Interest','#Revenge','#Revolution','#The Simpsons','#The Tomorrow People','#Trophy Wife','#Sleepy Hollow','#Two and a Half Men','#White Collar','#The Blacklist','#The Michael J Fox Show','#The Goldbergs']

	#	self=words = ['Koffee with Karan -RT -filter:links']
	#	self=words = ['#narendramodi -Congress : -RT', 'Koffee with Karan -RT']
	#	self=words = ['narendramodi Congress ?']
		#self=words = ['narendramodi-Congress :)']
		#self= words = ['narendramodi OR rahul']
...
#		file = open("observation.txt", "w")
		...
		for q in words:
			...
			try:
				...
				api = auth_()
				src = api.search(q, rpp=100, result_type="recent", count=20)
				for tweet in src:
					utc = pytz.utc
					home_tz = pytz.timezone(settings['timezone'])
					tweet_time = utc.localize(tweet.created_at).astimezone(home_tz)
					tweet.text = tweet.text.replace("\n", " ")
					i = i + 1
					l = (q + "\t" + tweet.user.screen_name.encode('utf-8') + "\t"  +
					tweet_time.strftime('%d/%m/%Y') + "\t" +
					tweet_time.strftime('%H:%M') + "\t" +
					tweet.text.encode('utf-8'))
					file.write(l + "\n")
					print i
					print l 
	...
	def do_stalk(self, q):
		...
		try:
			api = auth_()
			i = 0
			stk = api.user_timeline(q, count = 200, page = 1)
